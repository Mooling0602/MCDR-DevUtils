...
def main():
    ...
    meta["description"]["en_us"] = \
        get_input("请输入英文描述", meta["description"]["en_us"])
    meta["description"]["zh_cn"] = \
        get_input("请输入中文描述", meta["description"]["zh_cn"])
    meta["author"] = get_input("请输入作者", meta["author"])
    meta["link"] = get_input("请输入链接", meta["link"])
    meta["dependencies"]["mcdreforged"] = \
        get_input("请输入 mcdreforged 依赖版本", meta["dependencies"]["mcdreforged"])
